...
import { Search, Filter, ChevronUp, ChevronDown, ChevronsUpDown, Eye, Edit, Trash2, X, Download, MapPin, Package, Camera, QrCode } from 'lucide-react'
...
export default function RollInventory({
  ...
  const [viewMode, setViewMode] = useState<'inventory' | 'shipments'>(isQC ? 'shipments' : 'inventory')
  ...
  function handleQRScanSuccess(scannedData: string) {
    let targetRollId = scannedData.trim()

    // Try parsing JSON payload if QR code encodes JSON object from Incoming Roll step
    try {
      const parsed = JSON.parse(scannedData)
      if (parsed && typeof parsed === 'object') {
        targetRollId = String(parsed.roll || parsed.no_roll || targetRollId)
      }
    } catch (e) {
      // Raw string format
    }

    // Find matching roll in inventory
    const matched = rolls.find(r => {
      const q = targetRollId.toLowerCase()
      return (
        r.id.toLowerCase() === q ||
        r.no_roll?.toLowerCase() === q ||
        String(r.raw_id) === q ||
        ('r-' + r.raw_id).toLowerCase() === q ||
        r.jop?.toLowerCase() === q
      )
    })

    if (matched) {
      setLastScannedRoll(matched)
      if (!checkedRollIds.includes(matched.id)) {
        setCheckedRollIds(prev => [...prev, matched.id])
      }
      setRemovedShipmentIds(prev => prev.filter(item => item !== matched.id))
      setViewMode('shipments')
      SystemUI.toast({
        message: `Roll "${matched.id}" (${matched.grade}) scanned & added to Shipments!`,
        type: 'success'
      })
    } else {
      SystemUI.toast({
        message: `Scanned roll code "${targetRollId}" not found in inventory.`,
        type: 'warning',
        duration: 4000
      })
    }
    ...
    <div className="py-4 px-2.5 sm:px-6 space-y-4">
      <div className="flex flex-wrap justify-between items-center gap-2">
        ...
        <div className="flex gap-2 items-center">
          ...
          {isQC ? (
            <div className="flex items-center bg-blue-50 border border-blue-200 rounded-full px-4 py-1.5">
              <span className="text-xs font-extrabold text-blue-700 uppercase tracking-wide flex items-center gap-1.5">
                <Package size={13} /> Shipments Mode
              </span>
            </div>
          ) : (
      ...
      {/* Embedded Live Camera Scanner directly open at the top */}
      {(isQC || viewMode === 'shipments') && (
        <EmbeddedQRScanner
          onScanSuccess={handleQRScanSuccess}
          lastScannedRoll={lastScannedRoll}
        />
      )}
