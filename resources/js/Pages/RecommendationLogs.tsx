...
export default function RecommendationLogs({ logs = [], stats }: Props) {
  ...
  const [actionFilter, setActionFilter] = useState<'All' | 'ASSIGN' | 'MOVE'>('All')
  ...
  const computedStats: Stats = stats || {
    ...
    overall_match_rate: logs.length > 0 ? Math.round((logs.filter(l => l.is_match).length / logs.length) * 100) : 0,
    assign_total: logs.filter(l => l.action_type === 'ASSIGN').length,
    assign_match_rate: logs.filter(l => l.action_type === 'ASSIGN').length > 0
      ? Math.round((logs.filter(l => l.action_type === 'ASSIGN' && l.is_match).length / logs.filter(l => l.action_type === 'ASSIGN').length) * 100)
      : 0,
    move_total: logs.filter(l => l.action_type === 'MOVE').length,
    move_match_rate: logs.filter(l => l.action_type === 'MOVE').length > 0
      ? Math.round((logs.filter(l => l.action_type === 'MOVE' && l.is_match).length / logs.filter(l => l.action_type === 'MOVE').length) * 100)
      : 0,
  }
  ...
  const filteredLogs = logs.filter(log => {
    ...
    const matchSearch = !q ||
      ...
      (log.recommended_location && log.recommended_location.toLowerCase().includes(q)) ||
      (log.selected_location && log.selected_location.toLowerCase().includes(q)) ||
      (log.notes && log.notes.toLowerCase().includes(q))

    const matchAction = actionFilter === 'All' || log.action_type === actionFilter
    const matchStatus = statusFilter === 'All' || String(log.status_code) === statusFilter
  ...
  function handleRefresh() {
    ...
    router.reload({
      onFinish: () => {
        ...
        SystemUI.toast({ message: 'Recommendation logs refreshed.', type: 'success' })
      }
    })
  }
  ...
  function exportCSV() {
    if (logs.length === 0) {
      SystemUI.toast({ message: 'No logs available to export.', type: 'warning' })
      return
    }
    ...
    const csvRows = [
      ...
      ...logs.map(l => [
        ...
        `"${l.selected_location}"`,
        l.status_code,
        l.is_match ? '"MATCH (TRUE / 1)"' : '"OVERRIDE (FALSE / 0)"',
        `"${(l.notes || '').replace(/"/g, '""')}"`
      ].join(','))
    ]
    ...
    link.setAttribute('download', `rollyn_location_recommendation_logs_${new Date().toISOString().slice(0, 10)}.csv`)
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)

    SystemUI.toast({ message: `Exported ${logs.length} recommendation log records to CSV.`, type: 'success' })
  }
    ...
    <div className="py-4 px-2.5 sm:px-6 space-y-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Evaluasi akurasi dan kepatuhan operator terhadap rekomendasi penempatan lokasi slot gudang (Assign & Move)
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="card p-4.5 bg-gradient-to-br from-white to-blue-50/40 border border-blue-100 shadow-xs relative overflow-hidden">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Overall Match Rate</span>
          ...
          <div className="mt-2.5 flex items-baseline gap-2">
            ...
            <span className="text-xs font-semibold text-slate-500">
              ({computedStats.match_count} / {computedStats.total_evaluations} match)
            </span>
          </div>
          <div className="w-full bg-slate-200 h-1.5 rounded-full mt-3 overflow-hidden">
            <div
              className="bg-blue-600 h-full rounded-full transition-all duration-500"
              style={{ width: `${Math.min(computedStats.overall_match_rate, 100)}%` }}
            />
          </div>
          <div className="flex justify-between text-[11px] text-slate-400 mt-1.5 font-medium">
            <span>Boolean True (1): {computedStats.match_count}</span>
            <span>Override (0): {computedStats.override_count}</span>
          </div>
        </div>
        ...
        <div className="card p-4.5 bg-gradient-to-br from-white to-emerald-50/40 border border-emerald-100 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Assign Match Rate</span>
          ...
          <div className="w-full bg-slate-200 h-1.5 rounded-full mt-3 overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(computedStats.assign_match_rate, 100)}%` }}
            />
          </div>
        ...
        <div className="card p-4.5 bg-gradient-to-br from-white to-amber-50/40 border border-amber-100 shadow-xs">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-wider">Move Match Rate</span>
          ...
          <div className="w-full bg-slate-200 h-1.5 rounded-full mt-3 overflow-hidden">
            <div
              ...
              style={{ width: `${Math.min(computedStats.move_match_rate, 100)}%` }}
            />
          </div>
          <p className="text-[11px] text-slate-500 mt-1.5 font-medium">
      ...
      <div className="card p-4 sm:p-5 bg-white border border-slate-200/80 rounded-2xl shadow-xs space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pb-3 border-b border-slate-100">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs">
              <span className="text-slate-400 font-bold px-1 text-[10px] uppercase">Action:</span>
              {(['All', 'ASSIGN', 'MOVE'] as const).map(action => (
                <button
                  key={action}
            ...
            {/* Status Filter */}
            <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs">
              <span className="text-slate-400 font-bold px-1 text-[10px] uppercase">Evaluation:</span>
              <button
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="bg-slate-50/80 text-slate-500 font-bold uppercase tracking-wider border-b border-slate-200/80 text-[10.5px]">
                ...
                <th className="py-3 px-3.5">Recommended Slot</th>
                <th className="py-3 px-3.5">Selected Slot</th>
                <th className="py-3 px-3.5 text-center">Status / Boolean</th>
                ...
                <tr>
                  <td colSpan={9} className="py-12 text-center text-slate-400">
                    <div className="flex flex-col items-center justify-center gap-2">
                      <Sparkles size={32} className="text-slate-300 stroke-[1.5]" />
                      <p className="font-semibold text-slate-600">No recommendation evaluation logs found</p>
                    ...
                    >
                      ...
                      <td className="py-3 px-3.5 whitespace-nowrap">
                        <div className="font-semibold text-slate-800 flex items-center gap-1.5">
                          ...
                          <span>{log.created_date}</span>
                        </div>
                        <span className="text-[11px] text-slate-400 ml-4 font-mono">{log.created_time}</span>
                      </td>
                      ...
                      <td className="py-3 px-3.5 whitespace-nowrap">
                        <div className="flex items-center gap-1.5">
                          ...
                          <Link
                            href={log.no_roll ? `/roll-detail/${log.no_roll}` : '#'}
                      ...
                      <td className="py-3 px-3.5 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <div className="font-semibold text-slate-900 leading-tight">{log.user_name}</div>
                            {log.user_email && log.user_email !== '—' && (
                              <div className="text-[10px] text-slate-400 leading-tight">{log.user_email}</div>
                            )}
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-3.5 whitespace-nowrap">
                        <div className="inline-flex items-center gap-1 px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-800 border border-emerald-200 font-mono font-bold text-xs">
                          ...
                          <span>{log.recommended_location}</span>
                        </div>
                      </td>

                      {/* Selected Slot */}
                      <td className="py-3 px-3.5 whitespace-nowrap">
                        <div className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-lg font-mono font-bold text-xs border ${
                          isMatch
                            ? 'bg-blue-50 text-blue-800 border-blue-200'
                            : 'bg-rose-50 text-rose-800 border-rose-200'
                        }`}>
                          <MapPin size={11} className={isMatch ? 'text-blue-600' : 'text-rose-600'} />
                          <span>{log.selected_location}</span>
                        </div>
                      </td>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 text-xs text-slate-500">
          <div className="flex items-center gap-2">
            <span>Showing {filteredLogs.length > 0 ? (page - 1) * perPage + 1 : 0} to {Math.min(page * perPage, filteredLogs.length)} of {filteredLogs.length} entries</span>
            <select
              value={perPage}
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-3 sm:p-4">
          <div className="card w-full max-w-lg p-5 sm:p-6 bg-white rounded-2xl shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="flex justify-between items-start border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold text-sm ${
                  ...
                  {selectedLog.is_match ? <CheckCircle2 size={18} /> : <AlertTriangle size={18} />}
                </div>
                <div>
            ...
            <div className={`p-3.5 rounded-xl border flex items-center justify-between ${
              ...
              <div>
                <div className="text-xs font-bold uppercase tracking-wider">Evaluation Status</div>
                <div className="text-sm font-extrabold flex items-center gap-1.5 mt-0.5">
                  {selectedLog.is_match ? (
                    <>
                      <CheckCircle2 size={16} className="text-emerald-600" />
                      <span>MATCH (Boolean: 1 / True)</span>
                    </>
                  ) : (
                    <>
                      <AlertTriangle size={16} className="text-rose-600" />
                      <span>OVERRIDE (Boolean: 0 / False)</span>
                    </>
                  )}
                </div>
              </div>
              <span className={`px-2.5 py-1 rounded-lg text-xs font-extrabold ${
                selectedLog.is_match ? 'bg-emerald-600 text-white' : 'bg-rose-600 text-white'
              }`}>
                {selectedLog.is_match ? 'Accepted' : 'Manual Override'}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                ...
                <div className="text-[10px] text-slate-500">Sistem algoritma gudang</div>
              </div>
              ...
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl space-y-1">
                ...
                <div className={`text-base font-extrabold font-mono ${
                  selectedLog.is_match ? 'text-blue-700' : 'text-rose-700'
                }`}>
                  {selectedLog.selected_location}
                </div>
                <div className="text-[10px] text-slate-500">Pilihan aktual operator</div>
              </div>
            </div>
            ...
            <div className="space-y-2 text-xs bg-slate-50/50 p-3.5 rounded-xl border border-slate-100">
              <div className="flex justify-between py-1.5 border-b border-slate-200/60">
                ...
                <span className="font-bold text-blue-700 font-mono">{selectedLog.no_roll}</span>
              </div>
              <div className="flex justify-between py-1.5 border-b border-slate-200/60">
                <span className="text-slate-500 font-medium">Action Type</span>
                <span className="font-bold text-slate-800">{selectedLog.action_type}</span>
              </div>
              {selectedLog.action_type === 'MOVE' && (
                <div className="flex justify-between py-1.5 border-b border-slate-200/60">
                  <span className="text-slate-500 font-medium">Previous Slot</span>
                  <span className="font-bold text-slate-800 font-mono">{selectedLog.previous_location}</span>
                </div>
              )}
              <div className="flex justify-between py-1.5 border-b border-slate-200/60">
                <span className="text-slate-500 font-medium">Operator (PIC)</span>
              ...
              <div className="flex justify-between py-1.5">
                ...
                <span className="font-semibold text-slate-800">{selectedLog.created_at} WIB</span>
              </div>
              {selectedLog.notes && (
