...
export default function ProductionSchedule() {
    ...
    const previewStopTime = (() => {
        ...
        const stop = new Date(
            start.getTime() + previewProductionHours * 3600 * 1000,
        );
    ...
    function handleJopsIdChange(id: string) {
        const found =
            (jops as JopOption[]).find((j) => String(j.id) === id) || null;
    ...
    function openEditModal(row: ScheduleRow) {
        setEditId(row.id);
        const found =
            (jops as JopOption[]).find((j) => j.id === row.jops_id) || null;
        setSelectedJop(found);
        setForm({
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                    ...
                    <p className="text-xs text-slate-500 mt-0.5">
                        Schedule production runs based on existing SPK / Job
                        ...
                        {pagedRows.length > 0 ? (
                            pagedRows.map((r) => (
                                ...
                                >
                                    ...
                                    <td
                                        className={`text-xs ${r.remark ? "text-slate-700" : "text-slate-400 italic"}`}
                                        style={{ textAlign: "center" }}
                                    >
                                        {r.remark || "No notes"}
                                    </td>
                                        ...
                                        {isProduksi ? (
                                            r.status === 'OPEN' ? (
                                                <button
                                                    ...
                                                    className="btn btn-sm py-1 px-2 text-[11px] flex items-center gap-1 mx-auto bg-green-50 text-green-700 border border-green-200 hover:bg-green-100 cursor-pointer"
                                                >
                                                    <CheckCircle size={11} />
                                                    <span>Set Close</span>
                                                </button>
                                            ) : (
                                                <span className="text-[10px] font-bold text-slate-400 italic">CLOSED</span>
                                            )
                                        ) : (
                            ...
                            <tr>
                                ...
                                >
                                    No production schedules yet. Click{" "}
                                    <strong>Add Schedule</strong> to create one.
                                </td>
                            </tr>
                        )}
                    </tbody>
                    {rows.length > 0 && (
                        <tfoot>
                            <tr className="bg-amber-50 font-bold text-amber-900">
                                ...
                                <td className="py-2 text-center text-xs font-extrabold">
                                    {Number(totalTonnage).toFixed(2)} MT
                                </td>
                                <td colSpan={8} />
                            </tr>
                        </tfoot>
                ...
                <div className="flex gap-1">
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(
                        (p) => (
                            <button
                                key={p}
                                className={`btn btn-sm ${p === page ? "btn-primary" : "btn-secondary"} min-w-[30px] justify-center`}
                                onClick={() => setPage(p)}
                            >
                                {p}
                            </button>
