...
const stackCountOptions = ['Γ£ô', '2', '3', '4'];
...
const A_RACK_CONFIGS: RackConfig[] = [
  ...
  // A4: 1 ├ù 12 (col 1) + DOOR 3 ├ù 12 (cols 4,3,2)
          ...
          {rollsCount > 0 && (
            <div className="text-[10px] text-slate-400 border-t border-slate-700 mt-1 pt-1 space-y-0.5">
              {slot.rollsList.map((r, i) => (
                <div key={r.id}>
                  #{i + 1}: <span className="font-bold text-white font-mono">{r.number}</span> ({r.grade || 'ΓÇö'}, {r.weight || 0}kg)
                </div>
              ))}
            </div>
          )}
    ...
    <div className="py-4 px-2.5 sm:px-6 max-w-full overflow-x-hidden">
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Warehouse layout map Column A ΓÇö Capacity up to 4 rolls per slot</p>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs text-blue-700 truncate">
                Assigning roll <span className="font-bold font-mono">{assignRollNo}</span> ΓÇö Select an available slot (&lt; 4 rolls) on the map
              </div>
            </div>
          </div>
          <button
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 w-full">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <span className="text-[11px] text-slate-500 font-medium">{totalSlots} Slots ├ù Max 4 Rolls = {totalSlots * 4} Total Capacity</span>
                </div>
              </div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            ...
                            <div className="text-[10px] text-slate-500">
                              {roll.grade || 'ΓÇö'} ΓÇó {roll.weight || 0} kg
                            </div>
                          </div>
                        </div>
